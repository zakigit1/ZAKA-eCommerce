        ...
        <form action="{{route('admin.advertisement.homepage-banner-section-two')}}" method="post" enctype="multipart/form-data">
            ...
            @for($i=1 ;$i<=2 ;$i++)
                ...
                <div class="form-group">
                    ...
                    <input type="text" name="banner_url_{{$i}}" class="form-control"  placeholder="Banner URL" value="{{ @$homepageBannerSectionTwo[$i-1]->{'banner_'.$i}->{'banner_url_'.$i} }}">
                </div>
...
<div class="card border">
    @for($i=0 ;$i<=1 ;$i++)
        ...
        <div class="card-body text-center">
            @if (!isset($homepageBannerSectionTwo[$i]->{'banner_'.$i+1}->{'banner_image_'.$i+1}))
                <h3>No banners found</h3>
            @else
                
                <img src="{{asset($homepageBannerSectionTwo[$i]->{'banner_'.$i+1}->{'banner_image_'.$i+1})}}" 
