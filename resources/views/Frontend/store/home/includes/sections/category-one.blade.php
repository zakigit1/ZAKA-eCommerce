@php
    ...
    foreach ($productSliderSectionOne as $categoryType => $value){ //category type mean : sub_category or child_category
    ...
    if(array_keys($lastkey)[0] == 'category'){
        ...
        $products  =\App\Models\Product::where('category_id',$cat_type->id)->orderBy('id','DESC')->get();
    }elseif (array_keys($lastkey)[0] == 'sub_category') {
        $cat_type = \App\Models\Subcategory::find($lastkey['sub_category']);
        $products =\App\Models\Product::where('sub_category_id',$cat_type->id)->orderBy('id','DESC')->get();
    }elseif(array_keys($lastkey)[0]== 'child_category') {
        $cat_type = \App\Models\Childcategory::find($lastkey['child_category']);
        $products =\App\Models\Product::where('child_category_id',$cat_type->id)->orderBy('id','DESC')->get();
    }
